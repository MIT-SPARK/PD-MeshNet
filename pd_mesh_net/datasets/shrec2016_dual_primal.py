...
from torch_geometric.data import extract_tar, extract_zip
...
class Shrec2016DualPrimal(BaseDualPrimalDataset):
    r"""The SHREC 2016 dataset from the `"SHREC'16: Partial Matching of
    Deformable Shapes"
    <http://www.dais.unive.it/~shrec2016/shrec16-partial.pdf>`_ paper, in the
    format provided by `MeshCNN <https://github.com/ranahanocka/MeshCNN/>`_.
    The data is processed to form dual-primal graphs. Modified from
    `torch_geometric.datasets.SHREC2016`.

    Args:
        root (string): Root directory where the dataset should be saved.
        categories (list of string): The categories of the dataset from which to
            extract the data (valid values are: :obj:`"alien"`, :obj:`"ants"`,
            :obj:`"armadillo"`, :obj:`"bird1"`, :obj:`"bird2"`, :obj:`"camel"`,
            :obj:`"cat"`, :obj:`"centaur"`, :obj:`"dino_ske"`,
            :obj:`"dinosaur"`, :obj:`"dog1"`, :obj:`"dog2"`, :obj:`"flamingo"`,
            :obj:`"glasses"`, :obj:`"gorilla"`, :obj:`"hand"`, :obj:`"horse"`,
            :obj:`"lamp"`, :obj:`"laptop"`, :obj:`"man"`, :obj:`"myScissor"`,
            :obj:`"octopus"`, :obj:`"pliers"`, :obj:`"rabbit"`, :obj:`"santa"`,
            :obj:`"shark"`, :obj:`"snake"`, :obj:`"spiders"`,
            :obj:`"two_balls"`, :obj:`"woman"`). If an empty list, data are
            extracted from all the categories in the dataset.
        ...
        num_url (int, optional):
            - If 0: shrec_16 (16 training samples, 4 test samples) is used,
                version provided by MeshCNN;
            - If 1: shrec_16 (16 training samples, 4 test samples) is used,
                second version;
            - If 2: shrec_16 (16 training samples, 4 test samples) is used,
                third version;
            - If 3: shrec_10 (10 training samples, 10 test sample), first
                version is used;
            - If 4: shrec_10 (10 training samples, 10 test sample), second
                version is used;
            - If 5: shrec_10 (10 training samples, 10 test sample), third
                version is used.
            (default: :obj:`0`)
    ...
    urls = [
        ...
        'https://www.dropbox.com/s/5cadwa6hg0o751t/shrec_16_split_2.zip',
        'https://www.dropbox.com/s/zfxony8hotut8rv/shrec_16_split_3.zip',
        'https://www.dropbox.com/s/mcbuxd9nfj4bzzt/shrec_10_split_1.zip',
        'https://www.dropbox.com/s/mw2hi3nrf80nd8o/shrec_10_split_2.zip',
        'https://www.dropbox.com/s/ksemavuvfu3zigj/shrec_10_split_3.zip'
    ]

    valid_categories = sorted([
        'alien', 'ants', 'armadillo', 'bird1', 'bird2', 'camel', 'cat',
        'centaur', 'dino_ske', 'dinosaur', 'dog1', 'dog2', 'flamingo',
        'glasses', 'gorilla', 'hand', 'horse', 'lamp', 'laptop', 'man',
        'myScissor', 'octopus', 'pliers', 'rabbit', 'santa', 'shark', 'snake',
    ...
    def __init__(self,
        ...
        assert (num_url in [*range(6)])
        self.__num_url = num_url
        if (num_url in [0, 1, 2]):
            self.__subdivision_type = 'shrec_16'
        else:
            self.__subdivision_type = 'shrec_10'
        ...
        self.url = self.urls[num_url]
        super(Shrec2016DualPrimal, self).__init__(root, transform,
    ...
    def raw_file_names(self):
        ...
        return [
            osp.join(self.__subdivision_type, f"{cat}", "train")
            for cat in self.__categories
        ] + [
            osp.join(self.__subdivision_type, f"{cat}", "test")
            for cat in self.__categories
        ]
    ...
    def processed_file_names(self):
        ...
        if (should_obtain_processed_files_list):
            ...
            for category in self.__categories:
                paths = sorted(
                    glob.glob(
                        osp.join(self.raw_dir, self.__subdivision_type,
                                 category, f'{self.__split}/*.obj')))
    ...
    def process(self):
        ...
        for category, category_index in zip(self.__categories,
            ...
            paths = glob.glob(
                osp.join(self.raw_dir, self.__subdivision_type, category,
                         self.__split, '*.obj'))
